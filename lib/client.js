(function(root) {
  ...
  CrossStorageClient.prototype._poll = function() {
    ...
    interval = setInterval(function() {
      ...
      client._postMessage('cross-storage:poll');
    }, 1000);
  };
  ...
  CrossStorageClient.prototype._request = function(method, params) {
    ...
    return new this._promise(function(resolve, reject) {
      ...
      client._postMessage(JSON.stringify(req));
  ...
  CrossStorageClient.prototype._postMessage = function(message) {
    // postMessage requires that the target origin be set to "*" for "file://"
    var targetOrigin = (this._origin === 'file://') ? '*' : this._origin;

    // Send serialized message
    document.getElementById(this._frame.id).postMessage(message, targetOrigin);
  };
